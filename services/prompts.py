...
**{cultural_context}**
위 정보를 바탕으로, 현지인(Native)이 작성한 것처럼 해당 국가의 문화, 정서, 사회적 배경을 반영하여 콘텐츠를 최적화하세요. 단순히 언어를 번역하는 것이 아니라, 현지의 독자가 공감할 수 있는 표현과 사례를 사용해야 합니다.
...
- **플레이스홀더 절대 금지**: `[제품명]`, `[서비스명]`, `[AI Tool Name]`, `[Product Name]`, `[Tool Name 1]` 같은 대괄호 형식의 미완성 플레이스홀더 텍스트를 본문에 절대 사용하지 마세요. 반드시 소스 자료에 있는 실제 이름, 브랜드, 제품명을 그대로 사용하세요. 소스에 구체적인 이름이 없다면 일반적인 설명어로 대체하세요.
...
JSON만 반환하세요."""
    ...
    [요청 사항]
    1. 제목(title): 클릭을 유발하면서도 핵심 키워드를 포함한 강렬한 제목 (50자 이내)
    2. 카테고리(category): 글의 주제와 가장 잘 어울리는 카테고리 1~2개 (쉼표로 구분)
    3. 태그(tags): 검색 유입에 도움이 되는 핵심 키워드 5~10개 (배열 형식)
    4. 요약(summary): 글의 핵심 내용을 1~2문장으로 요약한 메타 설명 (150자 이내)
